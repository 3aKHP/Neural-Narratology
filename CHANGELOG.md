...
| **Prism Engine** | Phase III 自动化工具链（工程实现） | v10.3.3 Harness（面向 Prism Vesicle） |
...
## 2026-08-30 — Harness 10.3.3：HUD 数字契约校准

### Fixed — `[Modulation]`
- Runtime 与 Stage 的 HUD 抗机器化规则明确允许模板自带的轮次计数，同时继续禁止额外精确测量；Dyad State Navigator 移为独立顶层节。

### Changed — `[Modulation]`
- Harness/Driver 版本递增至 **10.3.3**；Vesicle Host Adapter `1.2.0`、引擎/Agent/资源/质量绑定与 `protocolVersion` 保持不变。
...
## 2026-08-22 — Harness 10.3.0-alpha.2：compose 变更面补齐

### Changed — `[Modulation]`
- Vesicle Host Adapter 的 `artifact.compose` 工具组补齐 `delete_file`、`move_file`、`move_directory`、`delete_directory`，与宿主基础提示词已声明的文件变更面保持一致；六个非 Stage 引擎与三个工作流 Agent（Scene Writer、Continuity Editor、Chapter Reviewer）经既有 `artifact.compose` 绑定获得上述工具。Harness/Driver 版本更新至 **10.3.0-alpha.2**；引擎、Agent、资源与质量绑定保持不变。

### Changed — `[Repo]`
- README 与 DELIVERY.md 同步 Harness 版本。
...
### Changed — `[Modulation]`
- Outline Schema Field Rules 同步 Transformation Directive 与 Register 字段说明；Outline 模板同步 Transform 与 Register 列。
- Harness/Driver 版本更新至 **10.2.1**。protocolVersion 保持 `v10.1-prompt-assembly`（装配机制未变）。Vesicle 侧零改动。

### Changed — `[Repo]`
- README、DELIVERY.md 同步版本号引用。
...
## 2026-07-20 — Stage Engine：七引擎矩阵，消费端沉浸式 RP

### Added — `[Modulation]`
- 新增 **Stage Engine**（七引擎矩阵第七引擎），定位为 Vesicle 内第一方消费端沉浸式 RP 入口。Stage 与 Runtime 共享三段式输出协议（Neural Chain + HUD + Prose），区别在交互范式：宿主 bootstrap 注入 Module A/B（无文件读取）、无 checkpoint 门控、quality observe。Neural Chain 以 HTML 注释包裹，消费端默认折叠、长按可查；HUD 低调 indicator。
...
- 校准语料统一增加 split、genre、source kind、model family、length bucket、POV、dialogue ratio 与 provenance。21 条 Guidance 正反例生成 42 条 tracked training case；Detector、Judge 与 host conformance 合计扩展至 102 条 JSONL case。
...
- CI 与 Harness Release workflow 使用冻结的 Bun lockfile 安装 schema-validation 依赖，继续执行 source check、contract tests 和两次 Harness 字节一致构建；Release 归档固定排序与时间，并拒绝覆盖指向其它提交的既有 tag。
...
| 2026-08-30 | v10.0 Tempered-Voice | **v10.3.2 Harness** | Dyad State Navigator 标题层级与数值例外澄清 |
| 2026-08-30 | v10.0 Tempered-Voice | **v10.3.3 Harness** | Runtime/Stage HUD 轮次数字契约校准 |
