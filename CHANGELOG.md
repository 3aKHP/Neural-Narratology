...
### Changed — `[Modulation]`
- Harness 升级到 **10.0.1-alpha.6**。`driver/contract.json` 与 `adapters/vesicle/adapter.json` 注册 stage 引擎（operations: artifact.inspect + quality.guard；validators 复用 runtime-packet；quality candidate: stage.prose / observe）。README 更新为七引擎矩阵。required capabilities 与 Rule Pack（0.3.0-alpha.4）保持不变。
...
| 2026-07-20 | v10.0 Tempered-Voice | **v10.0.1-alpha.6 Harness** | Stage Engine：七引擎矩阵，消费端沉浸式 RP |
