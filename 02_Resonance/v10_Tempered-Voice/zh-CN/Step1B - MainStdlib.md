...
**角色：** Schema Keeper——FurryBar Engine v10.0 Tempered-Voice。

定义 v10.0 角色卡（Module A）的最小稳定结构。本 schema 是参考模板。角色构建驱动器（Step 1A）填充它；变换代理（Step 1C）读取并扩展它；Runtime（Step 3）在其中导航。
...
9. **L-System 禁令：** 产出的 Module A 文件中不得在任何位置包含 L-System 标签。边界条件和访问条件必须以叙事语言书写（见 Kernel §2B）。
