...
| [`policies/`](./policies/) | 发布的 Semantic Judge rewrite Policy；当前 fixture 明确保持 inactive。 |
...
- 校准工具检查跨 split 精确/近重复泄漏，并在 ignored `dev/` 下生成、验证分离标签的
  blinded held-out bundle 与 SHA-256 freeze manifest。
- cn-antislop 固定快照只提供 DeepSeek 证据；8 个 L0/L1 候选保持 `evaluating`，
  当前没有候选进入 active detector rules。
- Rule Pack 同时要求 `quality-detector/document-metrics@1` 与
  `quality-judge/anti-ai-flavor@1`。Semantic Judge 只在 Vesicle Runtime 以 observe
  方式运行，finding 不进入自动重写策略。
- 发布 `quality-semantic-rewrite-policy/v1` schema 与 inactive fixture。它为模型精确
  scope、rule allowlist、最低置信度、inconclusive/unknown 与 multi-target 行为固定
  宿主合同；在 Policy 经过真实 calibration 和 preservation 验证、Harness 显式要求该
  capability 前，不授权 Semantic Judge rewrite。
...
# 校验 corpus provenance 与跨 split 泄漏
