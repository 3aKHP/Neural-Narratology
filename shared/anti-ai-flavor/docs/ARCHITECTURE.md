...
Pre-generation Guidance
  ...
  -> verified Semantic Rewrite Policy
...
`quality-semantic-rewrite-policy/v1` 是独立发布、按完整 protocol/model ID 精确匹配的
宿主策略制品。它只能 allowlist 已校准规则，并固定未知模型和 inconclusive 为 observe。
当前 fixture 为 inactive，不能单独使任何 finding 进入 rewrite；未来 Harness 只有在
真实 held-out 与 preservation 门槛通过后才会显式要求 `quality-policy/semantic-rewrite@1`。
...
保留议题包括英文 Guidance、production held-out 人类对照、跨模型候选复核、Judge rewrite
晋级阈值，以及 Scene/章节两级审计的费用平衡。
