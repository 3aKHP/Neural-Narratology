...
- 两次 Harness 构建字节一致。
- Harness/Driver 版本为 `10.3.0-alpha.1`，Runtime quality binding 为 `rewrite`；Dyad、
...
- Anti-AI-Flavor Rule Pack `0.4.0` 包含 Detector/Judge/calibration/annotation/
  rewrite/report/freeze JSON Schema、124 条 tracked calibration/conformance case、8 个
...
- 同包的 `quality-semantic-rewrite-policy/v1` fixture 处于 inactive，Harness 尚不要求
...
`10.0.1-alpha.5` 增加 inactive Semantic Rewrite Policy 合同，不改变宿主 required
capability 或 Runtime behavior。具体清单见 `VESICLE_ADAPTER_IMPLEMENTATION.md`。
