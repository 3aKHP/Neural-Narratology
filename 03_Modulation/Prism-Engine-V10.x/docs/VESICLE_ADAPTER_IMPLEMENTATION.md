...
当前 Agent Profile 的 tool allowlist 已按职责收紧。Vesicle 目前按工具名限制能力，尚未按委派路径限制单次写入目标。建议将委派输入中的 allowed write paths 作为 host-only scope 传给 child runner，并在文件工具执行层交叉校验。
...
3. quality binding → Output Quality Guard observe path。
4. SubAgent completion quality path。
5. `analyze_prose_quality` 与 Evaluate 接入。
6. HAL lifecycle-aware gate completion。
7. 长篇 Artifact Validator 与 child path scope。
