# Prism-Engine-V9.x-Installer

## 定位
- 用途：为 Prism Engine Universe v9.x 提供可分发的安装器、模板与 RooCode 模式补丁。
- 协议代际：v9.0 State-Space。
- 入口脚本：`Install.ps1`
- 支持两种安装模式：`Mode A`（模板内置 `.roo` 提示词）与 `Mode B`（全局 Rules Pack）。

## 目录说明
- `Install.ps1`：主安装脚本，负责复制模板、合并模式 YAML、按需安装 Rules Pack。
- `Prism-Engine-V9.x-Template/`：安装到 VSCode Project Templates 的工程模板骨架。
...
- 模板内 `specs/`、`templates/` 已同步包含 Persona Topology、Beat Map、DLC Schema、Lite persona prompt、Story Bible 与 Outline 资产。
...
## 引擎列表
v9.x 提供 6 个核心引擎：
1. `prism-etl`（新增 Workflow C：Affine Transform Agent）
2. `prism-runtime`（新增 State Navigator、第 11 条公理、5 行 HUD）
3. `prism-evaluate`（新增 Topology Coherence 维度 F）
4. `prism-weaver`
5. `prism-weaver-orch`
6. `prism-dyad`（更新为 beat_map 驱动）
...
## 安装后验证
- VSCode 的 Project Templates 中应出现 `Prism-Engine-V9.x-Template`
- RooCode 模式切换器中应出现：
  - `Prism ETL Engine`
...
## 相关文件
- 总说明：`03_Modulation/README.md`
- 引擎文档：`03_Modulation/Prism-Engine-V9.x/README.md`
- 模板说明：`03_Modulation/Prism-Engine-V9.x-Installer/Prism-Engine-V9.x-Template/README.md`
