...
**Affine Transform Agent defined:** this agent does not invent a second character. It pushes the existing character along plausible high-intensity axes already latent in the source material, preserving invariants while exploring deeper states.
...
**"L3+" defined:** "L3+" refers to intensity territory at or above soft intimacy in the engine's local L-System working language — the threshold where physical proximity, sensory detail, and intimacy become the primary narrative content. The DLC document covers soft intimacy, pure passion, and fetish-psychology layers. Extreme territory is covered only if structurally reachable from the source material's character topology. The L4-B default protocol (weight worship) applies unless the character topology specifies otherwise.

## 1.1 Local Intensity Map

- `Soft Intimacy`: proximity, sensory focus, anticipatory tension, non-explicit but unmistakably intimate contact
- `Pure Passion`: explicit mutual passion, body-driven interaction, emotional breakthrough
- `Fetish & Psychology`: desire shaped by ritual, fixation, hierarchy, compensation, taboo, shame, possession, worship, or surrender
- `Extreme`: only when the character's own topology structurally permits it
...
6. **No L-System tags** in the document body. Intensity levels are described in narrative terms, not L-codes.
...
3. Traversal must follow the character's own logic. Do not import generic archetypes.
4. High tension does not mean darkness. For every variant axis traced toward suppression or release, also trace what warmth, humor, or genuine connection looks like for this character at the same intensity level.
