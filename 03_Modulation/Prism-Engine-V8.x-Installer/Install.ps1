...
$TemplateSource = Join-Path $InstallerRoot "Prism-Engine-Universe-V8.0-Template"
...
function New-NoRooTemplateTempCopy() {
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("Prism-Engine-Universe-V8.0-Template-NoPrompts-" + [guid]::NewGuid().ToString("N"))
...
switch ($selectedMode) {
    "A" {
        ...
        $targetTemplate = Join-Path $ProjectTemplatesRoot "Prism-Engine-Universe-V8.0-Template"
